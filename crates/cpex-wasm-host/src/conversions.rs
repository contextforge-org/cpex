...
/// Converts native cpex-core MessagePayload to WIT MessagePayload for the sandbox.
...
/// Only maps fields that the WIT interface supports (request, security, http, meta).
...
pub fn wit_result_to_native(result: crate::sandbox_manager::types::PluginResult) -> NativePluginResult<native_msg::MessagePayload> {
    NativePluginResult {
        continue_processing: result.continue_processing,
        modified_payload: result.modified_payload.map(wit_payload_to_native),
...
fn wit_payload_to_native(payload: MessagePayload) -> native_msg::MessagePayload {
