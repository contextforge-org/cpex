...
- **`PipelineResult` gained a `payload_modified` field.** `modified_payload` is `Some` on every allowed pipeline, carrying the final payload whether or not a plugin touched it, so it never answered "did anything change?" — read the new flag for that. Construction goes through `allowed_with` / `denied` (plus a `with_payload_modified` builder), so callers using those are unaffected. (#151)
- **A pipeline field name reported to a plugin is now root-relative everywhere.** A `do:`-block field op passed `args.city` where the `args:` / `result:` sections passed `city`; both now pass `city`, with the phase selecting the root. Only matters to code reading `PluginInvocation::Field.name`. (#151)
