...
- **A field pipeline that conflicts with a plugin on the same path now logs the tie-break.** `args:` / `result:` pipeline edits take precedence over a plugin's edit to the same field (config-author-wins), and a key the plugin removed comes back if the pipeline rewrote it. Both cases warn with the field name instead of resolving silently. (#151)
