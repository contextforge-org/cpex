...
- Added isolated-environment compatibility for bare-FQN Python plugins, including
  automatic conversion with an opt-out, credential and extension handling, capability
  negotiation, and worker and installation reliability fixes
  ([#113](https://github.com/contextforge-org/cpex/pull/113)).

### Changed

- Migrated MCP transports to SDK 2.x and `httpx2`, including the split `mcp-types`
  package, updated SDK APIs and session handling, and HTTP host-binding and cleanup fixes
  ([#112](https://github.com/contextforge-org/cpex/pull/112)).

### Fixed

- Preserved safe, opt-in denial telemetry across in-process, MCP, gRPC, and Unix
  paths through immutable trusted outcomes and typed boolean and numeric metrics
  ([#180](https://github.com/contextforge-org/cpex/pull/180)).
